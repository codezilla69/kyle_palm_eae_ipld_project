...
min_temp = temps_df['AvgTemperatureFahrenheit'].min()  # TODO
max_temp = temps_df['AvgTemperatureFahrenheit'].max()  # TODO

min_temp_city = temps_df.loc[temps_df["AvgTemperatureFahrenheit"] == min_temp, "City"].values[0]  # TODO
min_temp_date = temps_df.loc[temps_df["AvgTemperatureFahrenheit"] == min_temp, "Date"].values[0]  # TODO

max_temp_city = temps_df.loc[temps_df["AvgTemperatureFahrenheit"] == max_temp, "City"].values[0]  # TODO
max_temp_date = temps_df.loc[temps_df["AvgTemperatureFahrenheit"] == max_temp, "Date"].values[0]  # TODO
...
if unique_countries_list is not None and len(selected_cities) > 0:
    ...
    for city in selected_cities:
        ...
        plt.plot(city_df_period["Date"], city_df_period["AvgTemperatureFahrenheit"], label=city)               # TODO plot each city line and use the label parameter to set the legend name for each city


    plt.title(f"Temperatures for Selected Cities from {start_date} to {end_date}")   # TODO
    plt.xlabel("Date")  # TODO
    plt.ylabel("Avgerage Temperature Fahrenheit")  # TODO
    ...
    for city in selected_cities:
        city_df = temps_df[temps_df["City"] == city]           # TODO: get a dataframe with the rows of the selected city
        city_df_period = city_df[(city_df["Date"] >= start_date) & (city_df["Date"] <= end_date)]    # TODO: get a dataframe with the rows of the selected city and the selected period of time using the Date column and any of the <, >, <=, >= operators to compare with start_date and end_date
        plt.hist(city_df_period["AvgTemperatureFahrenheit"], bins=20, alpha=0.5, label=city)                   # TODO: plot each city histogram in the same plot and use the label parameter to set the legend name for each city 

    plt.title(f"Temperature Distribution for Selected Cities from {start_date} to {end_date}")   # TODO
    plt.xlabel("Avgerage Temperature Fahrenheit")  # TODO
